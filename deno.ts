...
class QwenService {
    ...
    async translate(req: TranslateRequest): Promise<TranslateResponse> {
        ...
        for (const text of req.text) {
            const [result, detectedLang] = await this.translateSingleText(
                text,
                sourceLang,
                targetLang
            );
    ...
    async deeplxTranslate(req: DeepLXRequest): Promise<DeepLXResponse> {
        ...
        try {
            const [result, _] = await this.translateSingleText(
                req.text,
                sourceLang,
                targetLang
            );
    ...
    private mapLanguage(lang: string): string {
        const qwenLangMap: { [key: string]: string } = {
            "auto": "自动检测",
            "ZH": "简体中文",
            "EN": "英语",
        };
    ...
    private async attemptTranslation(
        ...
        let joinResult: QwenJoinResponse;
        try {
            joinResult = JSON.parse(respBody);
        } catch (error) {
            throw new Error(`failed to parse join response: ${error instanceof Error ? error.message : String(error)}, body: ${respBody}`);
        }
        ...
        const dataHeaders: { [key: string]: string } = {
    ...
    private async parseEventStream(response: Response): Promise<string> {
        if (!response.body) {
            throw new Error("No response body from data stream.");
        }
...
class AuthMiddleware {
    ...
    constructor() {
        this.enabled = Deno.env.get("AUTH_ENABLED") === "true";
    ...
    extractAPIKey(req: Request): string {
    ...
    unauthorized(req: Request): Response {
        ...
        } else if (path.includes("/v2/translate")) {
            return new Response(JSON.stringify({
    ...
    middleware(req: Request, handler: (req: Request) => Promise<Response>): Promise<Response> | Response {
        if (!this.enabled) {
            return handler(req);
        }
...
const handler = (req: Request): Promise<Response> | Response => {
    return corsMiddleware.middleware(req, async (req: Request): Promise<Response> => {
        const url = new URL(req.url);
...
export API_KEYS=sk-key1,sk-key2,sk-key3</pre>
            ...
            <details>
                ...
                <pre>curl -X POST https://xxx.deno.dev/translate \\
  ...
  "target_lang": "ZH"
            ...
            <details>
                ...
                <pre>curl -X POST "https://xxx.deno.dev/translate?api_key=sk-your-api-key" \\
  -H "Content-Type: application/json" \\
  -d '{
  "text": "Hello world",
  "source_lang": "auto",
  ...
  "target_lang": "ZH"
            ...
            <div class="endpoint">
                ...
                <details>
                    <summary>请求示例</summary>
                    <pre>curl -X POST https://xxx.deno.dev/v2/translate \\
  -H "Content-Type: application/json" \\
  -d '{
  "text": ["Hello world"],
  "source_lang": "auto",
  "target_lang": "ZH"
            ...
            <div class="endpoint">
                ...
                <details>
                    <summary>请求示例</summary>
                    <pre>curl -X POST https://xxx.deno.dev/v1/chat/completions \\
  -H "Content-Type: application/json" \\
  ...
  "translation_options": {
    ...
    "target_lang": "ZH"
            ...
            <div class="endpoint">
                ...
                <details>
                    <summary>请求示例</summary>
                    <pre>curl -X POST https://xxx.deno.dev/api/translate \\
  -H "Content-Type: application/json" \\
  -d '{
  "text": ["Hello world"],
  "source_lang": "auto",
  "target_lang": "ZH"
        ...
        <div class="version">
            ...
            <p>Modified by <a href="https://github.com/belingud">Belingud</a></p>
        ...
        try {
            if (url.pathname === "/translate") {
                return authMiddleware.middleware(req, async (req: Request): Promise<Response> => {
                    try {
                        const body = await req.json() as DeepLXRequest;
